...
function rankIcon(rank: string) {
  const r = rankName(rank).toLowerCase();

  if (r.includes("master") || r.includes("legend") || r.includes("champion")) {
    return "/combat_master.webp";
  }

  if (r.includes("elite") || r.includes("ace")) {
    return "/combat_ace.webp";
  }

  if (r.includes("knight") || r.includes("specialist")) {
    return "/combat_specialist.svg";
  }
...
function ProfileSkin({ player }: { player: any }) {
  return (
    <div className="relative h-[260px] w-[235px] overflow-hidden">
      <img
        src={skinBody(player)}
        alt={player?.ign || "player"}
        onError={(e) => {
...
export default function Home() {
  ...
  useEffect(() => {
    document.title = "BoltPvP Tierlist";
  ...
  const visibleOverallPlayers = useMemo(() => {
  ...
  const topTen = useMemo(() => rankedPlayers.slice(0, 10), [rankedPlayers]);

  const totalTests = useMemo(() => {
    return players.reduce((total, player) => {
      return (
        total +
        gameModes.filter((mode) => getTierData(player, mode.key).current !== "NT")
          .length
      );
    }, 0);
  }, [players]);
  ...
  const goldButton =
    "min-w-[260px] rounded-2xl border-2 border-yellow-400 bg-black/80 px-10 py-6 text-xl font-black tracking-[2px] text-yellow-200 shadow-[0_0_30px_rgba(255,200,0,0.28)] transition-all duration-300 hover:-translate-y-2 hover:scale-[1.05] hover:bg-yellow-400 hover:text-black hover:shadow-[0_0_45px_rgba(255,215,0,0.65)]";

  const discordButton =
    "inline-flex items-center justify-center gap-3 rounded-2xl bg-[#5865f2] px-8 py-4 text-lg font-black text-white shadow-[0_0_26px_rgba(88,101,242,0.35)] transition hover:-translate-y-1 hover:scale-[1.04] hover:bg-[#4752c4]";
      ...
      <div className="relative z-10 mx-auto min-h-screen max-w-[2000px] px-6 py-6 [zoom:0.9]">
        <header className="flex items-center justify-between rounded-2xl border border-yellow-500/20 bg-black/25 px-5 py-4 backdrop-blur-md">
          ...
          >
            <img
              src="/boltlogo.png"
              alt="BoltPvP"
              className="h-16 w-16 object-contain drop-shadow-[0_0_20px_rgba(255,200,0,0.8)]"
            />

            <div className="text-left">
              <h1 className="text-3xl font-black text-yellow-300">BoltPvP</h1>
              <p className="tracking-[5px] text-xs text-yellow-100">
                OFFICIAL TIERLIST
              </p>
            </div>
          </button>
          ...
          <nav className="flex items-center gap-4">
            <button
              onClick={() => goPage("home")}
              className={`rounded-xl px-8 py-4 text-2xl font-black transition-all duration-300 hover:-translate-y-1 hover:scale-110 hover:bg-yellow-400/10 hover:text-yellow-300 ${
                page === "home"
                  ? "text-yellow-300 underline decoration-yellow-300 decoration-4 underline-offset-8"
                  : "text-white"
              }`}
            >
              Home
        ...
        <div
          className={`transition-all duration-500 ease-out ${
            pageVisible ? "translate-y-0 opacity-100" : "translate-y-5 opacity-0"
          }`}
        >
          {page === "home" && (
            <section className="mx-auto grid min-h-[760px] max-w-[1680px] grid-cols-[minmax(0,1fr)_620px] items-center gap-12 px-6 py-12">
              <div className="max-w-4xl">
                <h2 className="max-w-6xl text-8xl font-black leading-[0.9] tracking-tight text-white drop-shadow-[0_0_34px_rgba(255,215,0,0.38)]">
                  <span className="text-yellow-300">BoltPvP</span>
                  <br />
                  Tier List
                </h2>

                <p className="mt-8 max-w-4xl text-3xl font-bold leading-[1.35] text-yellow-100/85">
                  Check the best players, view every gamemode tier, and apply for
                  official BoltPvP testing.
                </p>
                  ...
                  {[
                    {
                      icon: "🏆",
                      title: "Ranked Tiers",
                      text: "Track every official result across all modes.",
                    },
                    {
                      icon: "⚔️",
                      title: "PvP Modes",
                      text: "Sword, Axe, SMP, Mace, Crystal and Nethpot.",
                    },
                    {
                      icon: "⚡",
                      title: "Fast Testing",
                      text: "Apply through Discord and join the queue.",
                    },
                  ].map((card) => (
                    <div
                      key={card.title}
                      className="group relative overflow-hidden rounded-3xl border border-yellow-500/35 bg-[radial-gradient(circle_at_30%_20%,rgba(255,220,0,0.18),rgba(0,0,0,0.72)_55%)] p-6 shadow-[0_0_26px_rgba(255,190,0,0.16)] transition-all duration-300 hover:-translate-y-3 hover:scale-[1.04] hover:border-yellow-300 hover:shadow-[0_0_45px_rgba(255,210,0,0.45)]"
                    >
                      ...
                      <div className="relative z-10 mt-2 text-2xl font-black text-yellow-300">
                        {card.title}
                      </div>
                      <p className="relative z-10 mt-2 text-base font-semibold text-yellow-100/75">
                        {card.text}
                      </p>
                    </div>
                  ))}
                ...
                <div className="hero-float group relative mt-8 max-w-4xl overflow-hidden rounded-[36px] border border-yellow-400/45 bg-[radial-gradient(circle_at_20%_30%,rgba(255,211,0,0.16),rgba(0,0,0,0.78)_55%)] p-7 shadow-[0_0_45px_rgba(255,190,0,0.20)] transition-all duration-300 hover:-translate-y-3 hover:scale-[1.02] hover:border-yellow-300 hover:shadow-[0_0_60px_rgba(255,215,0,0.45)]">
                  ...
                  <div className="relative z-10 flex items-center gap-7">
                    <div className="relative h-36 w-36 overflow-hidden rounded-3xl border border-yellow-400/40 bg-yellow-400/10">
                      {topTen[0] ? (
                        <img
                          src={skinBody(topTen[0])}
                          alt={topTen[0].ign}
                          onError={(e) => {
                            e.currentTarget.src = fallbackBodySkin;
                          }}
                          className="absolute left-1/2 top-[-18px] h-52 w-52 -translate-x-1/2 object-contain"
                        />
                      ) : (
                        <img
                          src="/boltlogo.png"
                    ...
                    <div>
                      <div className="text-lg font-black tracking-[4px] text-yellow-300">
                        CURRENT #1 PLAYER
                      </div>
                      <div className="mt-2 text-5xl font-black text-white">
                        {topTen[0]?.ign || "Waiting..."}
                      </div>
                      <div className="mt-2 text-2xl font-black text-yellow-200">
                        {topTen[0]?.points || 0} points
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-10 flex flex-wrap gap-5">
                  <button onClick={() => goPage("tierlist")} className={goldButton}>
                    VIEW TIERLIST
                  </button>
                  <a
                ...
                <div className="mt-12 grid max-w-2xl grid-cols-2 gap-6">
                  <div className="group relative overflow-hidden rounded-3xl border border-yellow-500/35 bg-[radial-gradient(circle_at_50%_0%,rgba(255,215,0,0.18),rgba(0,0,0,0.78)_60%)] p-8 text-center shadow-[0_0_28px_rgba(255,180,0,0.14)]">
                    <div className="relative z-10 text-7xl font-black text-yellow-300 drop-shadow-[0_0_20px_rgba(255,215,0,0.4)]">
                      {players.length}
                    </div>
                    <div className="relative z-10 mt-2 text-xl font-black tracking-[4px] text-yellow-100/75">
                      TOTAL PLAYERS
                    </div>
                  </div>
              ...
              <aside className="rounded-[32px] border border-yellow-500/35 bg-[#081326]/90 p-8 shadow-[0_0_55px_rgba(255,190,0,0.18)] backdrop-blur-md">
                <h3 className="mb-7 text-center text-4xl font-black tracking-[4px] text-yellow-300">
                  TOP 10 PLAYERS
                </h3>
                ...
                <div className="space-y-3">
                  {topTen.map((player, index) => (
                    <button
                      ...
                      className="grid w-full grid-cols-[55px_70px_1fr_auto] items-center gap-4 rounded-2xl border border-yellow-500/20 bg-black/35 px-5 py-4 text-left transition hover:-translate-y-1 hover:border-yellow-300 hover:bg-yellow-500/10"
                    >
                      ...
                      <div className="relative h-16 w-16 overflow-hidden rounded-lg bg-yellow-400/10">
                        <img
                          ...
                          className="absolute left-1/2 top-[-12px] h-28 w-28 -translate-x-1/2 object-contain"
                        />
                      </div>
                      ...
                      <span>
                        ...
                        <span className="flex items-center gap-2 text-base text-blue-300">
                          <img
                            src={rankIcon(player.rank)}
                            alt={rankName(player.rank)}
                      ...
                      <span className="rounded-lg bg-yellow-400/15 px-4 py-3 text-lg font-black text-yellow-300">
                        {player.points || 0} pts
                      </span>
                    </button>
                  ))}
                  ...
                  {topTen.length === 0 && (
                    <div className="rounded-2xl border border-yellow-500/20 bg-black/35 py-12 text-center text-yellow-100/70">
                      No players yet.
                    </div>
                  )}
                </div>
                ...
                <button
                  onClick={() => goPage("tierlist")}
                  className="mt-6 w-full rounded-2xl border border-yellow-400/50 bg-yellow-400/10 px-6 py-5 text-2xl font-black text-yellow-300 transition hover:-translate-y-1 hover:scale-[1.02] hover:bg-yellow-400 hover:text-black"
                >
              ...
              <section className="mt-8 text-center">
                ...
                >
                  <img
                    src="/boltlogo.png"
                    alt="Bolt Logo"
                    className="mx-auto h-44 w-44 object-contain drop-shadow-[0_0_40px_rgba(255,210,0,0.85)]"
                  />
                </button>
                ...
                <div className="mx-auto mt-8 flex max-w-5xl flex-wrap justify-center gap-4">
                  {modes.map((mode) => (
                    <button
                      ...
                      className={`flex min-w-[150px] items-center justify-center gap-3 rounded-xl border-2 px-6 py-4 font-black transition hover:scale-105 ${
                        activeMode === mode.key
              ...
              {activeMode === "overall" && (
                ...
                >
                  ...
                  <div className="space-y-5 overflow-visible">
                    {visibleOverallPlayers.map((player) => {
                    ...
                    {visibleOverallPlayers.length === 0 && (
                      <div className="rounded-2xl border border-yellow-500/25 bg-black/40 py-16 text-center text-2xl font-black text-yellow-200">
                        No players found.
                      </div>
                    )}
                  </div>
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    <img
                                      ...
                                      alt={player.ign}
                                      onError={(e) => {
                                        e.currentTarget.src = fallbackHeadSkin;
                                      }}
        ...
        <footer className="mx-auto mt-20 max-w-[1500px] rounded-t-[34px] border-t border-blue-900/40 bg-[linear-gradient(90deg,#020816_0%,#031022_40%,#010814_100%)] px-8 py-14">
      ...
      {selectedPlayer && (
        ...
        >
          <div
            ...
            className="modal-pop relative w-full max-w-2xl scale-[0.9] rounded-[34px] border border-yellow-700 bg-[#140700] p-6"
          >
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
                ...
                <div className="flex items-center gap-2 rounded-xl bg-yellow-500/10 px-4 py-2 text-xl font-black text-yellow-300">
                  <img
                    src={rankIcon(selectedPlayer.rank)}
                    alt={rankName(selectedPlayer.rank)}
                    className="h-8 w-8 object-contain"
                    onError={(e) => {
                      e.currentTarget.style.display = "none";
                    }}
